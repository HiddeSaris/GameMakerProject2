draw_self();

var col;
if (m.can_build_now){
	col = c_lime;
}
else {
	col = c_red;
}

with obj_manager{
	if (mouse_check_button(mb_left)){
		if (building_state == building_states.building && building_conveyors){
			var _pos = building_conveyor_pos
			var sprite = sprite_buildings[selected_building];
			
			building_conveyors_dir = (abs(grid_x - _pos[0]) >= abs(grid_y - _pos[1]));
			
			if (grid_x == _pos[0] && grid_y == _pos[1]){
				draw_sprite_ext(sprite, selected_dir, other.x, other.y, 1, 1, 0, col, 1);
			}
			else if (building_conveyors_dir){ // x-direction
				var _len = grid_x - _pos[0]
				var _dir = -sign(_len) + 2;
				
				for (var _x=min(grid_x, _pos[0]); _x <= max(grid_x, _pos[0]); _x++){
					var _xx = grid_to_pos_x(_x, _pos[1]);
					var _yy = grid_to_pos_y(_x, _pos[1]);
					draw_sprite_ext(sprite, _dir, _xx, _yy, 1, 1, 0, col, 1);
				}
			}
			else{				// y-direction
				var _len = grid_y - _pos[1]
				var _dir = sign(_len) + 1;
				
				for (var _y=min(grid_y, _pos[1]); _y <= max(grid_y, _pos[1]); _y++){
					var _xx = grid_to_pos_x(_pos[0], _y);
					var _yy = grid_to_pos_y(_pos[0], _y);
					draw_sprite_ext(sprite, _dir, _xx, _yy, 1, 1, 0, col, 1);
				}
			}
		}
	}
	else if (building_state == building_states.building){ // draw preview
		switch(selected_building){
		case buildings.lumberjack:
			draw_area(lumberjack_get_area(grid_x, grid_y, 5, selected_dir), spr_indicator, c_white);
		break;
		case buildings.forester:
			draw_area(lumberjack_get_area(grid_x, grid_y, 5, selected_dir), spr_indicator, c_white);
		break;
		case buildings.farm:
			draw_circle_area(farm_get_area(grid_x, grid_y, farm_radius), spr_indicator, c_white);
		}
		draw_sprite_ext(sprite_buildings[selected_building], selected_dir, other.x, other.y, 1, 1, 0, col, 1);
	}
}
