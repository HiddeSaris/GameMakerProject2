...
//ParticleSystem
ps = part_system_create_layer(layer_get_id("Smoke_part"), false);
part_system_draw_order(ps, true);

//smoke
ptype1 = part_type_create();
part_type_shape(ptype1, pt_shape_smoke);
part_type_size(ptype1, 0.2, 0.2, 0.01, 0);
part_type_scale(ptype1, 1, 1);
part_type_speed(ptype1, 1, 1, 0, 0);
part_type_direction(ptype1, 80, 100, 0, 0);
part_type_gravity(ptype1, 0.01, 0);
part_type_orientation(ptype1, 0, 0, 0, 0, false);
part_type_colour3(ptype1, $4B4B5E, $000000, $000000);
part_type_alpha3(ptype1, 0.188, 0.161, 0);
part_type_blend(ptype1, false);
part_type_life(ptype1, 80, 180);

pemit1 = part_emitter_create(ps);
part_emitter_region(ps, pemit1, -8, 8, -1.0625, 13.0625, ps_shape_rectangle, ps_distr_linear);
part_emitter_stream(ps, pemit1, ptype1, 1);

switch (output_dir) {
	case dir.up:
		part_system_position(ps, x-36, y-10);
	break;
	case dir.right:
		part_system_position(ps, x-54, y-54);
	break;
	case dir.down:
		part_system_position(ps, x+52, y-54);
	break;
	case dir.left:
		part_system_position(ps, x+34, y-10);
	break;
	}
